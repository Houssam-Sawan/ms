...
function ceilCustom($number)
{
    $intVal = intval($number);
    if ($number - $intVal < .50) return (float)$intVal;
    return ceil($number);
}

function findingGrade($grades, $mark) {
    foreach ($grades as $grade) {
        if($grade->gradefrom <= $mark && $grade->gradeupto >= $mark) {
            return '<span class="text-bold">'.$grade->grade.'</span>';
        }
    }
    return false;
}
